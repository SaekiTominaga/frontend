...
export default class TooltipTrigger extends HTMLAnchorElement {
    #footnoteElement; // 脚注要素（ツールチップの内容をここからコピーする）
    #tooltipId;
    #tooltipWrapElement;
    #tooltipElement;
    #tooltipIdPrefix = 'tooltip-'; // ツールチップに設定する ID の接頭辞
    #mouseenterTimeoutId; // ツールチップを表示する際のタイマーの識別 ID（clearTimeout() で使用）
    #mouseleaveTimeoutId; // ツールチップを非表示にする際のタイマーの識別 ID（clearTimeout() で使用）
    #tooltipLabel; // ツールチップに設定するラベル
    #tooltipClass; // ツールチップに設定するクラス名
    #tooltipCloseText = 'Close'; // ツールチップの閉じるボタンのテキスト
    #tooltipCloseImageSrc; // ツールチップの閉じるボタンの画像パス
    #tooltipMouseenterDelay = 250; // mouseenter 時にツールチップを表示する遅延時間（ミリ秒）
    #tooltipMouseleaveDelay = 250; // mouseleave 時にツールチップを非表示にする遅延時間（ミリ秒）
    #tooltipCloseEventListener;
    #tooltipKeydownEventListener;
    constructor() {
        super();
    ...
    connectedCallback() {
        ...
        /* Image preload */
        if (tooltipCloseImageSrc !== undefined &&
